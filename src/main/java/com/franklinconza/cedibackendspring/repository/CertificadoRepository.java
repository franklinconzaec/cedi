...
public interface CertificadoRepository extends JpaRepository<Certificado, String> {
    ...
    @Query(value = "select case when rol.id='PONE' then 'I'||ins.id||'_P'||pon.id when rol.id='CONF' then 'I'||ins.id||'_C'||pon.id " +
            "when rol.id='INVE' then 'I'||ins.id||'_D'||pon.id else 'I'||ins.id end as id, " +
